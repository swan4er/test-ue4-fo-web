...
var Module = {
	...
	UE4_indexedDBVersion: 202101130022, // Bump this number to invalidate existing IDB storages in browsers.
};
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://cdn.rawgit.com/swan4er/test-ue4-fo-web/raw/master/HTML5/UE4Game.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://cdn.rawgit.com/swan4er/test-ue4-fo-web/master/HTML5/UE4Game.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://cdn.rawgit.com/swan4er/test-ue4-fo-web/master/HTML5/MyProject.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://cdn.rawgit.com/swan4er/test-ue4-fo-web/master/HTML5/Utility.js')).then(addScriptToDom);
		var dataDownload =
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://cdn.rawgit.com/swan4er/test-ue4-fo-web/raw/master/HTML5/MyProject.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://cdn.rawgit.com/swan4er/test-ue4-fo-web/raw/master/HTML5/MyProject.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
